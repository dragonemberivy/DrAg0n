...
function renderNews(base) {
  ...
  if (article && article !== 'news') {
    ...
    // Syntactic grammar engine for fully procedural story synthesis!
    const rand = getSeededRandom(articleName);
    const displayTitle = articleName.charAt(0).toUpperCase() + articleName.slice(1);
    const term = articleName.toLowerCase();

    // Vocabulary lists
    let nouns = ["database", "node", "ledger", "system", "register", "coordinate", "log", "record", "transaction", "portal", "token", "anomaly"];
    let adjectives = ["classified", "suspicious", "anomalous", "unauthorized", "quantum", "encrypted", "clandestine", "unidentified", "digital", "virtual"];
    let verbs = ["detected", "intercepted", "exposed", "monitored", "synchronized", "audited", "stabilized", "tracked", "analyzed", "validated"];
    let adverbs = ["rapidly", "unexpectedly", "officially", "successfully", "carefully", "closely", "silently"];
    let prepositions = ["near", "inside", "across", "under", "during", "corresponding to", "originating from"];

    // Add query words directly to nouns so they are naturally woven in
    const queryWords = articleName.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(w => w.length > 2);
    ...
    if (term.includes('cannibal') || term.includes('hannibal') || term.includes('eat') || term.includes('food') || term.includes('cook') || term.includes('flesh')) {
      nouns.push("chef", "recipe", "dinner", "kitchen", "psychiatrist", "cell", "plate", "gourmet", "tasting");
      adjectives.push("culinary", "exotic", "savory", "chilling", "macabre", "sophisticated");
      verbs.push("cooked", "served", "prepared", "escaped", "tasted", "consumed", "inspected");
    } else if (term.includes('space') || term.includes('planet') || term.includes('universe') || term.includes('galaxy') || term.includes('star') || term.includes('moon') || term.includes('orbit')) {
      nouns.push("rocket", "star", "nebula", "meteor", "alien", "orbit", "telescope", "observatory", "gravity");
      adjectives.push("galactic", "cosmic", "stellar", "astronomical", "void", "orbital");
      verbs.push("launched", "scanned", "detected", "collided", "orbited", "measured");
      ...
      nouns.push("bank", "vault", "treasury", "inflation", "wealth", "merchant", "market", "capital", "yield");
      adjectives.push("financial", "monetary", "commercial", "lucrative", "economic");
      verbs.push("traded", "deposited", "withdrawn", "accumulated", "purchased", "funded");
    }

    function makeSentence(sRand) {
      const structures = [
        () => {
          const art1 = seededPick(["A", "The"], sRand);
          const adj1 = seededPick(adjectives, sRand);
          const n1 = seededPick(nouns, sRand);
          const adv = seededPick(adverbs, sRand);
          const v = seededPick(verbs, sRand);
          const prep = seededPick(prepositions, sRand);
          const art2 = seededPick(["a", "the"], sRand);
          const adj2 = seededPick(adjectives, sRand);
          const n2 = seededPick(nouns, sRand);
          return `${art1} ${adj1} ${n1} ${adv} ${v} ${prep} ${art2} ${adj2} ${n2}.`;
        },
        () => {
          const prep = seededPick(prepositions, sRand);
          const art1 = seededPick(["a", "the"], sRand);
          const adj1 = seededPick(adjectives, sRand);
          const n1 = seededPick(nouns, sRand);
          const art2 = seededPick(["the", "a"], sRand);
          const n2 = seededPick(nouns, sRand);
          const v = seededPick(verbs, sRand);
          const art3 = seededPick(["a", "the"], sRand);
          const adj2 = seededPick(adjectives, sRand);
          const n3 = seededPick(nouns, sRand);
          const prepCap = prep.charAt(0).toUpperCase() + prep.slice(1);
          return `${prepCap} ${art1} ${adj1} ${n1}, ${art2} ${n2} ${v} ${art3} ${adj2} ${n3}.`;
        },
        () => {
          const art1 = seededPick(["The", "A"], sRand);
          const n1 = seededPick(nouns, sRand);
          const v1 = seededPick(verbs, sRand);
          const art2 = seededPick(["the", "a"], sRand);
          const adj1 = seededPick(adjectives, sRand);
          const n2 = seededPick(nouns, sRand);
          const connector = seededPick(["following reports that", "after logs revealed that", "when observations confirmed that"], sRand);
          const art3 = seededPick(["a", "the"], sRand);
          const adj2 = seededPick(adjectives, sRand);
          const n3 = seededPick(nouns, sRand);
          const v2 = seededPick(verbs, sRand);
          return `${art1} ${n1} ${v1} ${art2} ${adj1} ${n2} ${connector} ${art3} ${adj2} ${n3} was ${v2}.`;
        }
      ];
      const picker = Math.floor(sRand() * structures.length);
      return structures[picker]();
    }

    const sRand1 = getSeededRandom(articleName + "-p1");
    const sRand2 = getSeededRandom(articleName + "-p2");
    const sRand3 = getSeededRandom(articleName + "-p3");

    const p1 = `${makeSentence(sRand1)} ${makeSentence(sRand1)} ${makeSentence(sRand1)}`;
    const p2 = `${makeSentence(sRand2)} ${makeSentence(sRand2)} ${makeSentence(sRand2)}`;
    const p3 = `${makeSentence(sRand3)} ${makeSentence(sRand3)} ${makeSentence(sRand3)}`;
